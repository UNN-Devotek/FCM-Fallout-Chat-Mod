...
> **Current widget (2026-09-01):** `FCMChatWidget` v2.10.22 targets `/relay` through
...
### HUD identity cosmetics (widget v2.10.22)
...
`BitmapData.colorTransform` calls. When the local player sends a message, the widget waits for the
...
chat bridge strips unknown JSON members before they reach Scaleform, so v2.10.22 also reads a
