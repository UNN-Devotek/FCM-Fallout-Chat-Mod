...
**How it works:**  
`_startForegroundPoller()` is called from `startForegroundZOrder()` when `KDE_WAYLAND` or `IS_X11` is true. It probes for **both** tools (`command -v kdotool; command -v xdotool`) so the crash circuit-breaker (below) can fall back to the other one. `preferredForegroundTools()` sets the order: `kdotool` first on KDE-Wayland, `xdotool` first on X11.
