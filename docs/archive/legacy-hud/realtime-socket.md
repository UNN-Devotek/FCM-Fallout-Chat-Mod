...
> Historical generic socket transport. The June 2026 shipping decision below is superseded;
> FCMChatWidget uses native ZFE/xScal chat through `/relay`. Do not enable TCP/socket or remote-data
> settings to install the modern HUD. The old line protocol is distinct from the active
> `FCMHUD/1;...` metadata carrier. See [current integration](../../overlay/zfe/native-chat-relay/fcm-integration.md).

> **Historical decision (2026-06-24, superseded).** FCMHUD/1 (this `color~channel~user~content` push
...
> retired. The ZFE **`chat.v1`** native chat relay ([native-chat-relay/](../../overlay/zfe/native-chat-relay/README.md))
