...
```powershell
.\Packaging\release.ps1 -Version X.Y.Z -BridgeZip "<validated-0.2.9-prod.zip>" [-ReleaseNotes "..."] [-SkipBuild] [-DryRun] [-SkipWindowsNexus]
...
```powershell
.\Packaging\package-downloads.ps1 -Version X.Y.Z -BridgeZip "<validated-0.2.9-prod.zip>"
```
...
- `Fallout Chat Mod Portable X.Y.Z.zip` — portable guide plus optional bridge folder.
  Packaging fails closed unless the bundled bridge is the reviewed 0.2.9 candidate with ZIP
  SHA-256 `9c245bcad80b7baf1681d29273fda97008403a047bb941e00622a7210bb465af`
  and BA2 SHA-256 `9ae03b9e047c5fea221d10a8c805b0fe07cfcadb79af9667c766a4c627b54e44`.
  Native room/lag acceptance is still required before publishing this candidate.
...
- `FCM HUD Mod-<widget-version> (PROD).zip` — generated by
  ...
  version. Its `ZFE (Install for ZFE only)/` and `xScal (Install for xScal only)/` folders each contain the BA2,
  editable UI INI, provider configuration, a labeled `Data (drag the contents into data folder)/`
...
**HARD RULE — Nexus HUD ZIPs contain no executables or scripts.** The main website HUD ZIP
also contains no setup scripts. Before a Nexus HUD upload,
`publish-nexus-release.ps1` builds a separate `FCM HUD Mod-<widget-version> (PROD)-Nexus.zip`
with `--distribution nexus`. Both provider folders give manual setup in their own
`INSTALL.txt`; validation fails closed if a blocked
...
After publishing, verify that the tagged GitHub Release exists. In the 1.4.1 release,
the backend GitHub token returned HTTP 401, so the best-effort mirror did not create it;
the release was created separately with `gh release create v1.4.1` and the already
verified artifacts. If this recurs, fix or replace the token and create/update only
the missing GitHub Release. Do not repeat the admin publish or Discord announcement.
...
open only the selected `ZFE (Install for ZFE only)/` or `xScal (Install for xScal only)/` folder and follow its `INSTALL.txt`. A HUD-only
...
The default unified archive contains two identical BA2 copies, one inside each provider folder.
The ZFE folder includes the active fragment at its target-relative path; copy only the selected
folder's labeled Data folder contents into the game's `Data/` folder on a new install; do not
copy the labeled folder itself. Skip existing INIs and merge shared settings.
Updates replace only the BA2 and preserve edited INIs.
Set `hudModUrl` to this single ZIP. Older sibling-pair URLs remain supported for compatibility,
but new releases ship one download. xScal users merge the `xScal (Install for xScal only)/xscal.ini`
settings into their existing `xscal.ini`.
