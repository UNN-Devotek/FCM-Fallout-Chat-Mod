...
- **Manual release build:** `.github/workflows/build-windows.yml` — `workflow_dispatch` with `version` + `publish` inputs; runs on the **self-hosted** `[self-hosted, windows, unn]` runner by design. Release workflows (`build-windows.yml`, `build-linux.yml`) are NOT affected by the CI runner migration — they continue to use the self-hosted runners for consistent release toolchains.
- **CI gate (build):** `overlay-autoupdate-e2e-windows` in `ci.yml` — builds the NSIS installer **natively on `windows-latest`** (no Wine, no Docker, no docker-cp); defaults to GitHub-hosted `windows-latest`, switchable to self-hosted via `CI_RUNNER_WINDOWS`; runs on every PR and `prod`/`dev` push
- **CI gate (execution):** `overlay-autoupdate-e2e-windows-exec` in `ci.yml` — smoke-tests the built `.exe` natively on `windows-latest`; `continue-on-error` so it never blocks the run
- **Full failure history and fix rationale:** [`docs/testing/windows-nsis-ci-fixes.md`](docs/testing/windows-nsis-ci-fixes.md) — seven Wine/Docker failures documented in order, then the migration to native `windows-latest` (which superseded Wine/DinD entirely for CI). Read this before debugging any future Windows CI failure.
