...
export default function BugReports() {
  ...
  if (!isStaff) return <MyReports type="bug" />;
...
function BugReportsAdmin() {
  ...
  const { data: reports, isLoading } = useQuery({
    queryKey: ['player-reports', 'bug'],
    queryFn: () => api.get<BugReport[]>('/api/player-reports?type=bug'),
  });
  ...
  async function handleStatusChange(id: string, status: string) {
    ...
    try {
      ...
      queryClient.invalidateQueries({ queryKey: ['player-reports', 'bug'] });
    } catch {
      //
    } finally {
      setUpdatingId(null);
    ...
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minHeight: 0 }}>
      ...
      <div style={{ padding: '16px 0 12px', borderBottom: `1px solid ${border}` }}>
        ...
        <div style={{ fontSize: '11px', color: dim, marginTop: '4px', letterSpacing: '1px' }}>
          Technical issues and bug reports submitted via the web form
        </div>
      </div>
      ...
      <div style={{ flex: 1, overflow: 'auto', marginTop: '8px' }}>
        ...
        {!isLoading && (!reports || reports.length === 0) && (
          <div style={{ padding: '40px 0', textAlign: 'center', color: dim, letterSpacing: '2px', fontSize: '13px' }}>NO BUG REPORTS FOUND</div>
        )}
