...
class FcmEcho {
    ...
    public static function matches(messageId:String, senderUserId:String, channel:String, body:String,
            ...
            pendingBody:String, relayUserId:String, localUserId:String,
        ...
        var incomingIsLocal:Bool = isKnownLocal(senderUserId, relayUserId, localUserId, localLinkedUserId);
        if (incomingIsLocal) {
            return isKnownLocal(pendingSenderUserId, relayUserId, localUserId, localLinkedUserId)
                || pendingSenderUserId == null || pendingSenderUserId.length == 0;
        }
        ...
        return senderUserId == null || senderUserId.length == 0
            ? isKnownLocal(pendingSenderUserId, relayUserId, localUserId, localLinkedUserId)
            : false;
    }

    static function isKnownLocal(value:String, relayUserId:String, localUserId:String,
            localLinkedUserId:String):Bool {
        ...
        return (relayUserId != null && relayUserId.length > 0 && value == relayUserId)
            ...
            || (localLinkedUserId != null && localLinkedUserId.length > 0
                && value == localLinkedUserId);
    }
}
