...
> **Status (2026-09-01):** v2.10.16 — source, relay, and packaged BA2 are kept together. The
...
- Renders the server-resolved Overseer tag and vector-backed supporter marker in the HUD when the relay
  ...
  event members before the SWF receives them, so v2.10.16 decodes validated cosmetics from the
  `FCMHUD/1;...` envelope in the known empty `targetUserId` slot. Older widget builds receive
  no envelope. The marker is an inline Scaleform image rather than a font glyph, because the
